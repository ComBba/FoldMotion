...
> 작성일: 2026-09-16  
> 현재 코드베이스: M5 스토어 제출 문서. Play/Galaxy 콘솔 업로드는 계정 필요.
...
**상태: 구현** — Fluid / Hinge Shadow / Fade / Haptic + 강도(약·중·강) + 햅틱 토글. 기본값은 Fluid · 중(0.70).
...
- [x] Fold8 실기기에서 4스타일 모두 접기/펴기 왕복이 자연스럽다 (2026-09-16, 90° 프리셋 HUD 확인)
- [x] 설정 변경이 다음 접힘부터 즉시 반영된다 (`OverlayInput.settings` wake)
- [x] 기본값은 과하지 않은 Fluid + Strength 중간

---

## M5 — 스토어 출시

**상태: 제출 패키지 초안** — 정책 체크리스트·리스팅·개인정보 문서. 콘솔 업로드는 계정 대기.
...
- [ ] 내부 테스트 트랙에 빌드가 올라간다 (Play/Galaxy 계정 필요)
- [x] 정책 체크리스트(시스템 모방, Overlay, FGS, 접근성 오용 없음)를 문서화한다 (`docs/store-policy-checklist.md`)
- [x] 심사 반려 시 수정 포인트가 권한/카피 쪽으로 한정된다 (카피 초안 `docs/store-listing.md`, 정책 `docs/privacy.md`)
